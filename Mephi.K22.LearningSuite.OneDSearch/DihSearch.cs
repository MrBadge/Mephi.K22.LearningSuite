// Type: Mephi.K22.LearningSuite.OneDSearch.DihSearch
...
{
  [TaskClass("Дихотомия")]
  public class DihSearch : BaseSearch, ICheck
  {
    public DihSearch(Task task, RunMode mode, int retryNum)
      : base(task, mode, retryNum, ResultType.segment)
    {
    }

    public DihSearch(Task task)
      : base(task)
    {
    }
    ...
    {
      DihSearch dihSearch = new DihSearch(task, mode, retryNum);
    }
    ...
    {
      this.test = (BaseTest) new TestDih(((BaseSeacrhTaskObject) this.taskObject).Epsilon, ((DihTaskObject) this.taskObject).Left, ((DihTaskObject) this.taskObject).Right, ((DihTaskObject) this.taskObject).ExtremumType, ((DihTaskObject) this.taskObject).StopType, ((DihTaskObject) this.taskObject).MaxSteps);
    }
    ...
    {
      return (ICheck) new DihSearch(task);
    }
  }
}
