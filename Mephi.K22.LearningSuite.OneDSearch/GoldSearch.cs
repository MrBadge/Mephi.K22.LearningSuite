// Type: Mephi.K22.LearningSuite.OneDSearch.GoldSearch
...
{
  [TaskClass("Золотое сечение")]
  public class GoldSearch : BaseSearch, ICheck
  {
    public GoldSearch(Task task, RunMode mode, int retryNum)
      : base(task, mode, retryNum, ResultType.segment)
    {
    }

    public GoldSearch(Task task)
      : base(task)
    {
    }
    ...
    {
      GoldSearch goldSearch = new GoldSearch(task, mode, retryNum);
    }
    ...
    {
      this.test = (BaseTest) new TestGold(((BaseSeacrhTaskObject) this.taskObject).Epsilon, ((DihTaskObject) this.taskObject).Left, ((DihTaskObject) this.taskObject).Right, ((DihTaskObject) this.taskObject).ExtremumType, ((DihTaskObject) this.taskObject).StopType, ((DihTaskObject) this.taskObject).MaxSteps);
    }
    ...
    {
      return (ICheck) new GoldSearch(task);
    }
  }
}
