// Type: Mephi.K22.LearningSuite.OneDSearch.TestGold
...
{
  public class TestGold : BaseTest
  {
    ...
    public TestGold(double eps, double left, double right, ExtremumType eType, DihTaskObject.StopTypeEnum stopType, int maxStep)
    {
      this._eps = eps;
      ...
      if ((this.rigthFront - this.leftFront) * (2.0 / Constants.GoldNumber - 1.0) >= this._eps + Constants.DoublePrecision)
        return;
    ...
    {
      return (BaseTest) new TestGold(this._eps, this._leftPoint, this._rightPoint, this._extType, this._stopType, this._nmax)
      {
    ...
    public override ActionResult TestAction(Action act)
    {
      ...
      switch (act.ActionType)
      {
        ...
        case (byte) 9:
          ...
          if (this._stopType == DihTaskObject.StopTypeEnum.steps && this.step == 1 + this._nmax || this._stopType == DihTaskObject.StopTypeEnum.precise && (Math.Abs(this.prevTest - this.leftFront) / Constants.GoldNumber < this._eps + Constants.DoublePrecision || Math.Abs(this.prevTest - this.rigthFront) / Constants.GoldNumber < this._eps + Constants.DoublePrecision))
          {
      ...
      if (this._leftPoint <= coord && coord <= this._rightPoint)
      {
        if (Math.Abs(this._leftPoint - coord) > Constants.DoublePrecision)
        {
          if (Math.Abs(this._rightPoint - coord) > Constants.DoublePrecision)
          {
            ...
            {
              if (Math.Abs(this.leftFront + (this.rigthFront - this.leftFront) / Constants.GoldNumber - coord) < Constants.DoublePrecision || Math.Abs(this.rigthFront - (this.rigthFront - this.leftFront) / Constants.GoldNumber - coord) < Constants.DoublePrecision)
              {
                flag = true;
            ...
            else if (this.methodState == State.normal)
            {
              if (this.isDouble)
              {
                if (coord >= this.rigthFront)
                {
                  double num = this.prevTest - this.leftFront;
                  this.leftFront = this.prevTest;
                  this.prevTest = this.rigthFront;
                  this.rigthFront = this.rigthFront + num;
                }
                this.isDouble = false;
              }
              if (Math.Abs(coord - (this.leftFront + this.rigthFront - this.prevTest)) < Constants.DoublePrecision)
              {
                flag = true;
    ...
    private bool IsResultRight(double left, double right)
    {
      if (this.isDouble)
      {
        double num1 = this.rigthFront + this.prevTest - this.leftFront;
        double num2 = this.prevTest;
        return Math.Abs(this.leftFront - left) < Constants.DoublePrecision && Math.Abs(this.rigthFront - right) < Constants.DoublePrecision || Math.Abs(num2 - left) < Constants.DoublePrecision && Math.Abs(num1 - right) < Constants.DoublePrecision;
      }
      else
