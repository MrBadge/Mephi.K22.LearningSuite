// Type: Mephi.K22.LearningSuite.OneDSearch.CreateDiskTaskObject
...
{
  public class CreateDiskTaskObject : BaseTaskObjectControl
  {
    ...
    private TextBox tbDisk;

    public CreateDiskTaskObject(DiskTaskObject to)
    {
      ...
      foreach (double num in to.Points)
        str = !(str != string.Empty) ? str + string.Format("{0}", (object) num.ToString("0.00")) : str + string.Format("; {0}", (object) num.ToString("0.00"));
      this.tbDisk.Text = str;
    ...
    public CreateDiskTaskObject(DiskTaskObject to, bool enableEdit)
    {
      this.InitializeComponent();
      if (to != null)
      {
      ...
      this.tbName.ReadOnly = this.tbSegStart.ReadOnly = this.tbSegEnd.ReadOnly = this.tbDisk.ReadOnly = this.tbFunc.ReadOnly = true;
    ...
    private void InitializeComponent()
    {
      ...
      this.label5.Text = "Дискреты";
      this.label5.TextAlign = ContentAlignment.MiddleLeft;
      this.tbDisk.Location = new System.Drawing.Point(124, 96);
      this.tbDisk.Name = "tbDisk";
      this.tbDisk.Size = new Size(272, 20);
      this.tbDisk.TabIndex = 7;
      this.tbDisk.Text = "";
      this.tbDisk.TextAlign = HorizontalAlignment.Right;
      ...
      this.Controls.Add((Control) this.tbDisk);
      ...
      this.Name = "CreateDiskTaskObject";
    ...
    {
      DiskTaskObject diskTaskObject = (DiskTaskObject) null;
      try
      {
        string[] strArray = this.tbDisk.Text.Split(new char[1]
        {
          ';'
        });
        double[] points = new double[strArray.Length];
        for (int index = 0; index < strArray.Length; ++index)
          points[index] = double.Parse(strArray[index]);
        diskTaskObject = new DiskTaskObject(this.tbName.Text, double.Parse(this.tbSegStart.Text), double.Parse(this.tbSegEnd.Text), this.tbFunc.Text, points, this.rbMin.Checked ? ExtremumType.min : ExtremumType.max);
      }
      catch (Exception ex)
      {
        int num = (int) MessageBox.Show(ex.Message);
      }
      return (BaseTaskObject) diskTaskObject;
    }
