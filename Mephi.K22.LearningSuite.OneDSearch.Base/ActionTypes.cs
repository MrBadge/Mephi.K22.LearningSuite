// Type: Mephi.K22.LearningSuite.OneDSearch.Base.ActionTypes
// Assembly: Mephi.K22.LearningSuite.OneDSearch.Base, Version=0.1.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 0EF8375E-BF87-46B7-A32A-E286B4EDBF9E
// Assembly location: C:\Dropbox\MEPhI\Optimization_methods\Одном\Mephi.K22.LearningSuite.OneDSearch.Base.dll

namespace Mephi.K22.LearningSuite.OneDSearch.Base
{
  public enum ActionTypes
  {
    CreatePointByCoord,
    CreatePointByDiv2Points,
    CreatePointByPointSegment,
    CreateSegmentByLen,
    CreateSegmentBy2Points,
    CreateSegmentByDiv2Segment,
    DeletePoint,
    DeleteSegment,
    RunTest,
    SetResult,
    CreateSegmentByGoldSegment,
    CreateSegmentByFib,
    CreatePointByPaul,
    CreateConnectedSegment,
    CreateFictDiskrete,
    RunTestFictive,
  }
}
